...
const TableRow: FC<TableRowProps> = ({ courseID, professorIDs, section, index }) => {
  ...
  return (
    <tr key={index}>
      {professorIDs?.length && <td className="data-col">{courseID}</td>}
...
const Schedule: FC<ScheduleProps> = ({ course, professorIDs, terms }) => {
  ...
  const fetchScheduleDataFromAPI = useCallback(async () => {
    const apiResponse: WebsocAPIResponse = course
  ...
  }, [course, professorIDs, selectedQuarter]);
    ...
    <div>
      ...
      <div className="table-wrapper">
        <table className="ppc-table schedule-table">
          <thead>
            <tr>
              {professorIDs?.length && <th>Course</th>}
              <th>Code</th>
              <th>Section</th>
              <th>Units</th>
              <th>Instructor</th>
          ...
          <tbody>
            {Object.entries(scheduleData).flatMap(([courseID, sections]) => {
              return sections.map((section, i) => (
                <TableRow key={i} courseID={courseID} professorIDs={professorIDs} section={section} index={i} />
              ));
            })}
          </tbody>
        </table>
