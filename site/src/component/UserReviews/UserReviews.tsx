...
const UserReviews: FC = () => {
  ...
  const [professorData, setProfessorData] = useState<
    Map<string, ProfessorGQLData>
  >(new Map());
  const [courseData, setCourseData] = useState<Map<string, CourseGQLData>>(
    new Map()
  );
    ...
    (async () => {
      ...
      for (const review of reviews) {
        ...
        if (!courseData.has(courseID)) {
          await searchAPIResult('course', courseID).then((course) => {
            if (course) {
        ...
        if (!professorData.has(profId)) {
          await searchAPIResult('professor', profId).then((professor) => {
            if (professor) {
